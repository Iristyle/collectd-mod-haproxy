...
static void hap_submit_counter (const char *svname, const char *pxname, const char *type, int len, ...) {
	value_t *values;
	va_list ap;
	int i;
...
static void hap_notification(const char *svname, const char *pxname, const char *type, int severity, const char *message) {
	...
	snprintf(notif.type_instance, sizeof (notif.type_instance), "%s-%s", svname, pxname);
	plugin_dispatch_notification(&notif);
		
}

static void hap_submit_gauge (const char *svname, const char *pxname, const char *type, int len, ...) {
	value_t *values;
	va_list ap;
	int i;
	value_list_t vl = VALUE_LIST_INIT;
...
static int hap_retrieveuptime(unsigned long *uptime) {
	...
	*uptime = 0;
        ...
        if ( connect (s_fd, (const struct sockaddr *)&address, sizeof(address)) < 0 ) {
                goto err;
        }

        if ( send(s_fd, HAP_SHOW_INFO, sizeof(HAP_SHOW_INFO)-1, 0) < (sizeof(HAP_SHOW_INFO)-1) ) {
                goto err;
        }
...
static int hap_retrievestat(void) {
	...
	int ret = -1;
        ...
        if ( send(s_fd, HAP_SHOW_STAT, sizeof(HAP_SHOW_STAT)-1, 0) < (sizeof(HAP_SHOW_STAT)-1) ) {
		goto err;
        }
...
hap_status_t *new_hap_status(const char *svname, const char *pxname, int status) {
...
static int hap_read (void)
{
	...
	while ( pentry ) {
			...
			if ( pentry->status == 0.0F ) {
				...
				if ( !pstatus ) {
					...
					if ( hap_notifs & HAP_NOTIF_STATUSDOWN ) {
						hap_notification(pentry->svname, pentry->pxname, "hap_status", 
							NOTIF_WARNING,
							"DOWN");
					}
		...
		if ( hap_flags & HAP_BYTES ) {
			hap_submit_counter (pentry->svname, pentry->pxname, "hap_bytes", 2,
				pentry->bin,
				pentry->bout);
		}

		if ( hap_flags & HAP_SESSIONS ) { 
			hap_submit_counter (pentry->svname, pentry->pxname, "hap_sessions", 2, 
                               	pentry->stot,
                               	pentry->rate);
               	}

		if ( hap_flags & HAP_ERRORS ) {
               	        hap_submit_counter (pentry->svname, pentry->pxname, "hap_errors", 3,
                                pentry->ereq,
                       	        pentry->ersp, pentry->econ);
               	}
		
		if ( hap_flags & HAP_DENY ) {
                        hap_submit_counter (pentry->svname, pentry->pxname, "hap_deny", 2,
               	                pentry->dreq,
                       	        pentry->drsp);
               	}

		if ( hap_flags & HAP_STATUS ) {
                        hap_submit_gauge (pentry->svname, pentry->pxname, "hap_status", 1,
               	                pentry->status);
               	}
filtered:
